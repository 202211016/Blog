...
export class db {
	...
	async executeQuery(query: string,values: any[] = []) {
		...
		try {
			...
			if (result === null || result.rows === null) {
				return [];  // Return an empty array if result or result.rows is null
			}

			if (result.command == "INSERT") {
				if (this.uniqueField != '') return result['rows'][0][this.uniqueField];
	 ...
	 * @returns array
	 */
	select(table: string, fields: string, where: string, orderby: string, limit: string ,params: any[] = []) {
...
async listRecords(fields = '*', params: any[] = []) {
	 ...
	 * @param fields fields
	 */
	async allRecords(fields: string = '*', condition: string = '') {
		let result = await this.select(this.table, fields, condition, this.orderby, '');
		return !result ? [] : result;
	}
...
// /**
//  * Retrieve a single record with the given condition
//  * @param fields Fields to be selected
//  * @returns Object containing the single record
//  */
// async getSingleRecord(fields = '*') {
//     let result = await this.select(this.table, fields, this.where, this.orderby, 'LIMIT 1');
